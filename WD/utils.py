...
def comp_area_lat_lon(lat: np.array, lon: np.array) -> np.array:
    ...
    lat_edge[1:nlat] = 0.5 * (lat[: nlat - 1] + lat[1:nlat])
    lat_edge[nlat] = min(
        90, lat[nlat - 1] - 0.5 * (lat[nlat - 2] - lat[nlat - 1])
    )
