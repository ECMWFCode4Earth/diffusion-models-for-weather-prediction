...
class AFNO2D(nn.Module):
    ...
    def forward(self, x):
        # expects different shape for inputs and outputs: (batchsize, height, width, n_channels)
        bias = x  # residual connection

        dtype = x.dtype
        x = x.float()  # convert to float if double
        B, H, W, C = x.shape

        x = torch.fft.rfft2(x, dim=(1, 2), norm="ortho")
        x = x.reshape(B, H, W // 2 + 1, self.num_blocks, self.block_size)  # W // 2 + 1? -> to remove redundant information
        ...
        # the signs differ between real and imaginary parts. This can be explained by imagining w1[0] and w1[1] as real and imaginary part of a complex weight w1.
        ...
        x = x.type(dtype)  # convert back to original datatype

        return x + bias  # add residual connection
